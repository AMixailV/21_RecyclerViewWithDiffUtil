...
class UsersService {
    ...
    init {
        ...
        ) }.toMutableList() // создается map-ом просто List, поэтому надо привести к мутабл
    }
    ...
    fun deleteUser(user: User) {
        ...
        if (indexToDelete != -1) {
            users = ArrayList(users) // В UsersService привязываем ссылку users к новой копии старого списка,
                                     // и из этой копии удаляем элемент, чтобы diffUtil смог увидеть разницу
                                     // при работе с базой данных этого не придется делать
            users.removeAt(indexToDelete)
            notifyChanges()
        }
    }

    // Если moveBy == 1, то перемещаем пользователь вверх в списке, если -1,то вниз.
    fun moveUser(user: User, moveBy: Int) {
        ...
        users = ArrayList(users)// В UsersService привязываем ссылку users к новой копии старого списка,
                                // и из этой копии удаляем элемент, чтобы diffUtil смог увидеть разницу
                                // при работе сбазой данных этого не придется делать
    ...
    fun fireUser(user: User) {
        ...
        if (index == -1) return
        val updatedUser = users[index].copy(company = "") // т.к. User дата класс, то в методе copy() можно изменить какие-то поля
